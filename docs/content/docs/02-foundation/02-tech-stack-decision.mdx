...
但 `Our100` 这种项目，真正昂贵的地方不是多写几层视图，而是：
...
`Our100` 需要真正的宿主能力，比如活跃 App、输入强度、托盘、自启动、系统通知、安全存储。这类能力放在 [Tauri 2](https://v2.tauri.app/blog/tauri-20/) 上是顺的，因为它本来就适合“Web 前端 + Rust 系统能力”这种组合，而且它的 [capabilities 权限模型](https://v2.tauri.app/security/capabilities/) 也很适合后面拿来讲宿主边界。
...
我不会把 `Tauri Mobile` 放在主方案位置，不是因为它完全不能做，而是因为它对 `Our100` 来说，赌注有点太高。
...
问题就在这里。`Our100` 的移动端不是一个普通表单 App，它要碰的是：
...
对 `Our100` 这种“JS/TS 为主，但关键能力必须按需下沉原生”的产品，这比“勉强统一宿主方案”更值钱。
:::
...
`Our100` 的服务端，我建议采用 TypeScript 体系。
...
后端主框架我还是选 `NestJS`，因为模块化、依赖注入、工程组织、多人协作这些点，对 `Our100` 这种主线项目还是够稳。
...
我这里不建议现在走 GraphQL，也不建议全量押 `tRPC`。`Our100` 后面会有多端、回放、联调、自动化验证、SDK 生成这些需求，接口契约最好保持稳定和可审计。
...
`Our100` 不是那种“先把功能做出来，隐私以后再补”的项目。
