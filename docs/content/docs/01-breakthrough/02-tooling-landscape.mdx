...
description: 不做单选题，用岗位分工、模型分工和作者自己的实战习惯构建可控组合
icon: Wrench
---
...
这也是为什么我不会在这一章里大吹“云端 agent 全自动接项目”这条路。
...
这篇文章本身，就是我用 GPT-5.4 编写的。之所以会把 GPT-5.4 拉进这类任务里，一个重要原因是 OpenAI 在 [Introducing GPT-5.4](https://openai.com/index/introducing-gpt-5-4/) 里明确强调了两点：它是 *most capable and efficient frontier model for professional work*，同时也是 *most token efficient reasoning model yet*。这只是官方说法，不是我替官方做结论，但在文档、分析、专业写作这类任务上，我确实愿意把它和 Claude Opus 4.6 换着用。
