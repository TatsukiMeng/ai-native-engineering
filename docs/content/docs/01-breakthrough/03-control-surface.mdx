---
title: 从 Prompt 到 Context 再到 Agent Harness：AI 编程控制面的进化
description: 理解 Prompt、Context 与 Agent Harness 的分层关系，建立 AI Native 开发的控制面心智
icon: Layers
---

# 这一章不再叫 Context Engineering，因为我自己中途改主意了

最开始这第 3 章，其实不是这么定的。

当时我是先根据自己的经验，和 AI 一起把 outline 过了一遍，初步定下来写 `Context Engineering`。因为按我原来的直觉，这一章讲清楚上下文、规则、污染、约束，差不多也就够了。
...
这也是我为什么会中途改主意：这一章如果还只是写成狭义的 `Context Engineering` 教程，就有点低估问题了。
...
所以这一章不该再写成狭义的 `Context Engineering` 教程。

这一章真正要讲清楚的是：**AI 编程的控制面，正在从 Prompt 扩到 Context，再扩到 Agent Harness。**
...
所以这一章不会装成它是一个早就行业统一定义好的经典术语。我这里采用这个词，是因为它确实很有解释力，能把“Prompt 不够、Context 也不够，真正缺的是整套执行壳”这件事讲清楚。
:::
...
这一节我建议用一个统一任务贯穿，不然这一章很容易飘成概念体操。
...
如果这一章写到这里读者还只是记住了 `Prompt`、`Context`、`Harness` 三个词，那这一章其实还没写成功。

因为这一章真正想完成的，不是再塞三个新名词，而是让人突然意识到：

**原来我前面写的那些模型、那些开发方式、那些工具形态、那些生态位，不是在铺陈信息量，而是在铺这条控制面的主线。**

## 第 1 章为什么要先写那么多模型、交互方式和开发模式

现在回头看第 1 章《从"你写 AI 补"到"你说 AI 做"的范式转移》，应该就会更清楚。
...
所以第 2 章为什么要写“岗位分工”？因为如果没有岗位分工，读者最后就会把所有东西都混成一句话：

> 找个最强的工具，然后希望它包打天下。

但真实工程从来不是这么赢的。

## 第 3 章真正做的，是把前两章的线收成一张图

如果硬要把前三章压成一句话，那大概就是：

- 第 1 章回答：**为什么范式变了**
- 第 2 章回答：**现在有哪些岗位和武器**
- 第 3 章回答：**这些东西到底该怎么在脑子里串成一个控制面**

到了这里，前两章那些看上去分散的内容，其实才真正闭环：
...
**原来前两章写的不是散装知识，而是在铺今天这一张控制面总图。**

# 这也是为什么下一章不再继续聊“选什么”，而是开始聊“怎么别翻车”
...
这时候，下一章为什么要讲“暗礁”，其实就很自然了。

因为一旦开始真实落地，问题就不再只是“会不会用 AI”，而是：
...
换句话说，这一章不是为了把第一章封死，而是为了把读者送到下一章真正的工程问题里。

:::important[如果这一章真的讲明白了，读者应该开始这样想]
...
- 下一章开始讲“暗礁”，则是因为真正的工程难点，从来都不是会不会生成，而是怎么避免失控
...
- 哪些是在帮自己治理信息
- 哪些是在帮自己稳定执行
- 哪些又会在下一章里变成调试成本、护栏设计和失败兜底的问题来源

这时候，第一章才算真的从“破局”走到了“收束”。
